<template>
  <div class="maket">
    ...
    <div class="form">
      <x-input title=" <span>*</span>  姓名：" required="required" v-model="mName" placeholder="请输入" is-type="china-name"></x-input>

      <x-input title="  <span>*</span> 手机号：" required="required" mask="99999999999" v-model="mPoints" placeholder="请输入" :max="11"
  ...
  export default {
    ...
    methods: {
      ...
      next: function () {
       ...
       if(this.mName=='' || this.mPoints=='' || this.mCompany==''|| this.mDepartment==''){
         AlertModule.show({
           title: '请填写完整资料',
         })
         return false
       }
       let url=this.HOST + '/mv/visit/reserveForInterviewee'
        this.$axios.post(url,{
